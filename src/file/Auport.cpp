...
auDtype
fmt_type_to_dtype (uint16_t fmt_type, uint16_t bit_depth) {
    switch (fmt_type) {
    case 1:
...
uint16_t
dtype_to_fmt_type (auDtype dtype) {
    switch (dtype) {
    case auDtype::uInt:
    case auDtype::sInt:
        return 1;
...
bool
auFileReader::get_error () {
    return error;
}
uint32_t
auFileReader::get_duration () {
    return duration;
}
uint32_t
auFileReader::get_buf_size () {
    return buf_size;
}
auSFormat
auFileReader::get_s_format () {
    return s_format;
}

bool
auFileReader::read_chunk (char *buffer, size_t size) {
...
bool
auFileWriter::write_chunk (char *buffer, size_t size) {
